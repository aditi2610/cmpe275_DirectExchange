...
public interface TransactionsRepository extends JpaRepository<Transactions, Long>{
	...
	@Query(value= "select SUM(t.sending_amount/c.rate) as convertedUSD , SUM(servie_fee/c.rate) as totalFee, count(DISTINCT t.offer) as totalOffers, t.status as status from transactions t inner join currency_rates c on c.currency_code  = t.source_currency where t.creation_date >= date_Add(now(), interval -12 month) and t.status=1", nativeQuery = true)
	...
	@Query(value= "select SUM(t.sending_amount/c.rate) as convertedUSD , SUM(servie_fee/c.rate) as totalFee, count(DISTINCT t.offer) as totalOffers, t.status as status from transactions t inner join currency_rates c on c.currency_code  = t.source_currency where t.creation_date >= date_Add(now(), interval -12 month) and t.expiration_date >= NOW() and t.status=0", nativeQuery = true)
	public List<Object[]> generatePendingTransactionReport();
